  ...
  if (!SAFARI) {
	...
	var youtubeSubscription = false;  //Must be global as the listener needs to be accessed 
	var jsonYTSubs=new Array();
	...
	function checkLogin(){
		...
		chrome.identity.getAuthToken({ 'interactive': true }, function(token) {
			//If the subscriptions aren't set, set them
			if(storage_get('youtube_subscriptions') == null){
				sendAjax("");
			}
		});
	}
	...
	chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
		if(storage_get('youtube_subscriptions').indexOf(request.id) !== -1){
			youtubeSubscription=true;
		}else{
			youtubeSubscription=false;
		}
		return true;
	});
		...
		jQuery.ajax({
				...
				success:function(jsonObj){
						for (var i=0;i<jsonObj.items.length;i++){
							//Add Subscriptions to the stored list (for access elsewhere)
    ...
    function onBeforeRequestHandler(details) {
	  ...
	  if(frameDomain === "www.youtube.com"){
			//Check the user is logged in
			checkLogin();
			if(youtubeSubscription==true){
				return { cancel: false }; //Enable video ads
			}
	  }
      // Issue 7178
  ...
  if (STATS.firstRun) {
	// Deny install from AdBlock website
